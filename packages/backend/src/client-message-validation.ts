...
const ClientMessageSchema = z.discriminatedUnion("type", [
  ...
  messageSchema("missions:repo-profiles:list", {
    requestId: z.string(),
  }),
  messageSchema("missions:repo-profiles:upsert", {
    requestId: z.string(),
    profile: z.object({
      ...
      defaultBuildWorkingDirectory: z.string().nullable().optional(),
      defaultRegistry: z.string().nullable().optional(),
      registryHints: z.array(z.string()).optional(),
      requiredEnvVars: z.array(z.string()).optional(),
      requiredSdks: z.array(z.string()).optional(),
      userFacingCopyGlobs: z.array(z.string()).optional(),
      uiTestGlobs: z.array(z.string()).optional(),
      notes: z.string().nullable().optional(),
      source: z.enum(["builtin", "user", "learned"]).optional(),
    }),
  }),
  messageSchema("missions:repo-profiles:delete", {
    requestId: z.string(),
    projectKey: z.string().min(1),
  }),
  messageSchema("missions:validation-profiles:list", {
    requestId: z.string(),
  }),
  messageSchema("missions:validation-profiles:upsert", {
    requestId: z.string(),
    profile: z.object({
      id: z.string().optional(),
      projectKey: z.string().nullable().optional(),
      ...
      kind: z.enum(["build", "unit-test", "lint", "typecheck", "restore", "format", "e2e-smoke"]),
      command: z.string().min(1),
      workingDirectory: z.string().min(1),
      notes: z.string().nullable().optional(),
      confidence: z.number().min(0).max(1).optional(),
      expectedRuntimeMs: z.number().int().nonnegative().nullable().optional(),
      prerequisites: z.array(z.string()).optional(),
    }),
  }),
  messageSchema("missions:validation-profiles:delete", {
    requestId: z.string(),
