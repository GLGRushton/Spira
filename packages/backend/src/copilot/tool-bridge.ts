import { type CopilotSession, type ToolResultObject, defineTool } from "@github/copilot-sdk";
...
const buildTool = (tool: McpTool, aggregator: McpToolAggregator) =>
  defineTool(tool.name, {
    ...
    skipPermission: tool.annotations?.readOnlyHint === true && tool.annotations?.destructiveHint !== true,
    handler: async (args) => {
      try {
...
export function registerTools(session: CopilotSession, aggregator: McpToolAggregator): void {
